...
async def run():
    ...
    rows = await conn.fetch("SELECT report_id, firm_nm, article_title, report_url, reg_dt FROM source_reports WHERE firm_nm = '미래에셋' ORDER BY reg_dt DESC LIMIT 10")
    for row in rows:
        print(f"ID: {row['report_id']}, Title: {row['article_title']}, URL: {row['report_url']}, Date: {row['reg_dt']}")
