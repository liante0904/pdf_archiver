...
from legacy.v1.pdf_archiver_async import PDFArchiver, get_pdf_page_count
...
async def archive_id_1495():
    ...
    target_path = archiver._make_file_path(row['firm_nm'], row['article_title'], row['report_date'], row['report_id'])
    ...
    if target_path.exists() and target_path.stat().st_size > 1024:
        ...
        await conn.execute(
            ...
            1495, row['firm_nm'], row['article_title'], str(rel_path), target_path.stat().st_size, pages, row['report_date']
        )
